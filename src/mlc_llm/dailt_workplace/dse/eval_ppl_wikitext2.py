        ...
        writer.writerow(
            {
                ...
                "stride": stride,
                "split": split,
                "load_in_4bit": int(load_in_4bit),
                "load_in_8bit": int(load_in_8bit),
                "fake_q3bit": int(fake_q3bit),
                "ppl": ppl,
            }
        )
...
def main():
    parser = argparse.ArgumentParser("在文本数据集上计算与 MLC 模型对应的 HF 模型 PPL（默认 WikiText-2）")
    parser.add_argument(
        "--mlc-model",
        type=str,
    ...
    parser.add_argument(
        ...
        help="评估用的数据集：wikitext2 或 wikitext103",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    # 5) 把结果追加写入 CSV，方便后续汇总不同模型 / 量化的 PPL
    # 统一写在脚本所在目录，且按数据集区分文件
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ppl_csv = os.path.join(script_dir, f"{args.dataset}_ppl_results.csv")
    append_ppl_result_row(
        csv_path=ppl_csv,
