...
class DispatchKVCacheCreation:  # pylint: disable=too-many-instance-attributes
    ...
    def transform_module(self, mod: IRModule, _ctx: tvm.transform.PassContext) -> IRModule:
        ...
        # 简单 DSE：在不改算法的前提下，仅缩小关键容量参数，验证参数调整是否能正常跑通。
        # 注意：这里只做非常保守的修改，将值减半但不小于 1，并打印修改后的结果。
        prefill_before = kwargs["prefill_chunk_size"]
        max_batch_before = kwargs["max_batch_size"]

        if isinstance(kwargs["prefill_chunk_size"], tvm.tir.IntImm):
            v = kwargs["prefill_chunk_size"].value
            kwargs["prefill_chunk_size"] = tvm.tir.IntImm(
                kwargs["prefill_chunk_size"].dtype, max(1, v // 2)
            )
        if isinstance(kwargs["max_batch_size"], tvm.tir.IntImm):
            v = kwargs["max_batch_size"].value
            kwargs["max_batch_size"] = tvm.tir.IntImm(
                kwargs["max_batch_size"].dtype, max(1, v // 2)
            )

        # 打印修改前后数值，方便确认 DSE 是否生效
        try:
            print(
                "[KV DSE] max_batch_size: {} -> {}, prefill_chunk_size: {} -> {}".format(
                    getattr(max_batch_before, "value", max_batch_before),
                    getattr(kwargs["max_batch_size"], "value", kwargs["max_batch_size"]),
                    getattr(prefill_before, "value", prefill_before),
                    getattr(kwargs["prefill_chunk_size"], "value", kwargs["prefill_chunk_size"]),
                ),
                flush=True,
            )
        except Exception:
